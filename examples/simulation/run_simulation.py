...
Solar gains are loaded from data/profiles/internal_gains.csv (Q_sol column),
which was computed from the measured DWD irradiance data in
data/solar/hannover_solar_2024_2026.csv (period Nov 2024 – May 2026) plus
synthetic clear-sky estimates for the earlier period.
Re-run data/solar/calc_solar_gains.py to regenerate the solar gains.
"""
...
def simulate_baseline(building, heatcurve, pid, weather, gains, dt=3600):
    ...
    for i in range(n_steps):
        ...
        Q_gains = gains.iloc[i]['Q_int_total']      # includes internal + solar gains
...
def simulate_mpc(building, mpc, weather, gains, prices, dt=3600):
    ...
    for i in range(n_steps):
        ...
        if i % 24 == 0:
            # Get forecasts – Q_int_total now includes solar gains
