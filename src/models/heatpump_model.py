...
class i4b_Heatpump_AW:
    """
    i4b-style Dimplex LA 6TU air-water heat pump model.

    Adapted from i4b Heatpump_AW.
    Original i4b interface uses COP(T_hp, T_amb).
    This adapter provides compute_COP(T_amb, T_flow) so it works with our Simulator.
    """

    def __init__(self):
        self.name = "i4b_Heatpump_AW"

        # Operating limits from i4b comments / manufacturer data
        self.T_amb_min = -22.0
        self.T_amb_max = 35.0
        self.T_flow_min = 18.0
        self.T_flow_max = 60.0

    def compute_COP(self, T_amb, T_flow):
        """
        COP polynomial fit from i4b Heatpump_AW.

        Parameters
        ----------
        T_amb  : ambient/source temperature [°C]
        T_flow : heat pump supply temperature [°C]
        """
        T_hp = T_flow
        ...
        return float(np.clip(COP, 1.0, 8.0))

    def is_within_operating_limits(self, T_amb, T_flow):
        return (
            self.T_amb_min <= T_amb <= self.T_amb_max
            and self.T_flow_min <= T_flow <= self.T_flow_max
        )


class i4b_Heatpump_Vitocal:
    """
    i4b-style Vitocal ground-source / water-water heat pump model.

    Adapted from i4b Heatpump_Vitocal.
    Original i4b interface uses COP(T_sink, T_amb), with estimated ground temperature.
    This adapter provides compute_COP(T_amb, T_flow) so it works with our Simulator.
    """

    def __init__(self):
        self.name = "i4b_Heatpump_Vitocal"
    ...
    def _t_ground_from_amb(self, T_amb):
        """
        Simple approximation for ground temperature from ambient temperature.

        i4b imports this from src.disturbances.t_ground_from_amb.
        We keep a local approximation to avoid changing your project imports.
        """
        return 0.5 * T_amb + 5.0

    def compute_COP(self, T_amb, T_flow):
        """
        COP polynomial fit from i4b Heatpump_Vitocal.

        Parameters
        ----------
        T_amb  : ambient temperature [°C], used to estimate ground temperature
        T_flow : heat pump supply temperature [°C]
        """
        T_sink = T_flow
        T_source = self._t_ground_from_amb(T_amb)
...
# ── Collection of available heat pump models ──────────────────────────────────
HEATPUMP_MODELS = [
    iDM_AERO_ALM_4_12,
    i4b_Heatpump_AW,
    i4b_Heatpump_Vitocal,
]

HEATPUMPS = {
    "idm": iDM_AERO_ALM_4_12,
    "airwater": i4b_Heatpump_AW,
    "vitocal": i4b_Heatpump_Vitocal,
}
